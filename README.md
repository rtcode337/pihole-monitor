...
- **調べた結果を2つに分けて記録できる**（任意でメモを残せる）——
  **問題あり**（広告・トラッカー等で、ブロックされて当然）と
  **問題なし**（怪しい候補として挙がったが、調べたら無害だった＝誤検知）。
  一覧は 未確認 / 問題あり / 問題なし / すべて で絞り込める
- **メモは確認済みにしなくても残せる**（調べただけの段階と、人が確認した段階は別）
- **チェックボックスで選んだ複数行をまとめて確認済みにできる**（メモはそのまま残る）
- **聞き方が2つある。** ツールバーの「まとめてAIに聞く」は、**チェックした全員**に何十件ぶんの
  短い説明を書かせて読み比べる。行の「詳しく調べる」は、**メインの1人**が
  **web検索とPi-holeの観測データ**でその1件を深く調べる（運営元・用途・通信の頻度・放置してよいか）。
  **結果はメモを上書きせず**、行を押すと開く詳細画面にメモとは別枠で出る
- **まとめて聞くのは、既定ではメモが空のものだけ**（空いているところを埋める）。
  チェックを入れるとメモがあるものも作り直す
- **聞く相手を画面から選べる（何人でも。「詳しく調べる」を頼むメインの1人も指定する）**。選んだ全員に同じ内容を聞き、**答えを「誰が書いたか」付きで1つのメモに並べる**（Chiezo に登録してある Claude Code / Codex / … と、同梱の Claude Code CLI から選ぶ。再起動なしで反映）
...
- **DNSクエリを定期的に取り込んで蓄積する**（起動時に過去30日ぶんのドメインの初出も遡って埋める）
- **ブロックされていない通信の中から怪しい候補を出す**（上段のタブ「未ブロックの監視」）。
  **はじめて見たドメイン**・**存在しない名前が返り続けているドメイン**・**珍しいクエリ種別**・
  **規則正しい周期で鳴っている通信（どの端末かも出す）**・**毎回ちがう長い名前（DNSトンネリングの形）**で拾い、
  **なぜ挙がったかを行に添える**。候補にもメモ・判定・「詳しく調べる」がそのまま使える
- **理由を押すと、Pi-holeのクエリログでその通信だけを絞り込んで開く**（周期なら鳴らしている端末まで）。
  **どうやって候補を選んでいるか**（5つの手としきい値）も監視の前置きから読める
- **調査結果をもとに、詳細画面から追加で質問できる**（メインの1人が、これまでのやり取りと観測データを踏まえて答える）
- **設定画面から ping / 経路（tracepath）を打てる**。一覧に並ぶのは「名前を引いた記録」だけなので、
  その先に本当に届くのかは実際にパケットを出さないと分からない。
  **結果は1行ずつ出る**（終わるまで待たない）。経路はIPに加えて**引ける名前も添える**
- **スマホで下に引っ張ると更新できる**
- **監視の基準日時を決められる**（「この日時より後だけを見る」）。ネットワークの設定を変えた日は、
  変える前の通信が別の環境の記録になってノイズにしかならないので、そこを境に切れる
