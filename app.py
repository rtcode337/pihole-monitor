...
async function loadDomains() {
  ...
  try {
    ...
    document.getElementById('domain-list').innerHTML = '<div class="empty"><div class="empty-icon">&#9888;</div>Pi-holeへの接続に失敗しました</div>';
  }
}
...
document.addEventListener('keydown', e => {
  ...
  if (e.key === 'Enter' && e.ctrlKey && document.getElementById('modal').style.display !== 'none') submitReview();
});
