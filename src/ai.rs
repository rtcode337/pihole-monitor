...
/// 選択の置き場(`settings` 表のキー)。値は [`AiChoice`] の配列の JSON ——
...
///
/// 相手(経路)では変えないが、材料が違えば聞くことも違う ——
/// ブロック済みの一覧は「Pi-hole が止めたもの」なので「なぜ止まったか」を聞けばよいが、
/// 監視の一覧はブロックの結果ではなく、通信の振る舞いから選んだ候補で、
...
impl AskMode {
    ...
    fn origin(self) -> &'static str {
        match self {
            ...
            Self::Watch => "このドメインは、ブロックリストに載っていたから挙がったものではありません。\
                Pi-hole が記録したDNSの問い合わせの中から、振る舞い(はじめて見た・NXDOMAINが多い・\
                珍しいクエリ種別・規則正しい間隔・毎回ちがう長い名前)で自動的に拾った候補です。\
                ブロックの有無は判断材料に入れていないので、Pi-hole が止めているものも混ざります。\
                止められているかどうかは、渡した観測に「Pi-holeが止めている」とあるかで判断し、\
                無ければブロックされたと決めつけないでください。",
        }
    }
}
...
///
/// 1件でもまとめてでも同じ指示・同じ形式(番号付きの入力とJSONの応答)。詳細の「AIに聞く」で
/// 1件だけ聞くときも答えはそのままメモになるので、出力の形を変える理由が無い ——
/// 文言を2つ持つと、1件のときとまとめたときでメモの書き方が変わる。
///
/// 出力は一覧に並ぶメモなので1〜2文に抑えさせる(3〜5行あると読めない。
/// 相手が複数なら人数ぶん並ぶので、短さの意味はさらに大きい)。
...
const SYSTEM_PROMPT_WATCH: &str = "あなたはDNSとネットワークに詳しい技術者です。\
    ...
    ブロックの有無では絞っていないので、Pi-hole が止めているものも混ざります。\
    各行の括弧の中に「Pi-holeが止めている」とあればそれを踏まえてください。\
    無ければ「ブロックされた」「ブロックされて当然」のような、ブロックされたことを前提にした\
    書き方はしないでください。\
...
/// 1件を深く調べるためのもので、web 検索と、こちらが渡す観測データの両方を使わせる。
...
/// ([`one_liner`])。一覧に出す用途を指示文に書いておくことで、
...
/// 決まった見出しで一通り書かせるもの。こちらはすでに書いた内容を材料に、
/// 利用者の1つの問いへ答えるだけなので、見出しを付けさせると同じことがもう一度並ぶ。
///
...
pub struct AiChoice {
    ...
    /// `default` にしてあるので、この列より前に保存した選択もそのまま読める
...
/// 抜き出した「ひとこと」の長さの上限(文字)。一覧に並ぶメモなので、
/// 長いと行が読めなくなる。
...
pub struct Investigation {
    ...
    pub research: String,
    /// 一覧に出す「ひとこと」(`[書き手] 1〜2文`)。メモが空のときだけ書く用。
...
impl Ai {
    ...
    /// ドメインについて選んだ相手全員に聞く(1件でも複数でも同じ経路)。
    ///
    ...
    async fn ask_target(
        ...
        let (text, author) = if target.is_cli() {
            ...
            // Chiezo が未設定なら選択から落としてあるので、ここに来るのは設定済みのとき
            ...
            // 名前は応答が名乗ったものを優先する。こちらがモデルを指定していないとき、
            // 何が書いたのかを知る手がかりはそれだけ
    ...
    /// web 検索と観測データの両方を渡すので、通常の問い合わせよりずっと時間がかかる
    /// (上限は `INVESTIGATE_TIMEOUT`)。
    ///
    /// `mode` はどちらの一覧から押されたか。ブロック済みなら「止まっている」、
...
/// メモが空のときはここをメモにする。
///
/// 見出しの後ろに続けて書かれることも、次の行から書かれることもあるので、
...
mod tests {
    ...
    fn watch_prompt_does_not_assume_the_domain_was_blocked() {
        ...
        // ただしブロックの有無で絞っていないので止められているものも混ざり、
        // その行は理由の文に「Pi-holeが止めている」が入る(app.js の reasonText)。
        // 「止まったものではない」と言い切ると、その行で指示と観測が食い違う
        let watch = AskMode::Watch.system_prompt();
        assert!(watch.contains("ブロックされたことを前提にした"));
        assert!(watch.contains("Pi-holeが止めている"));
        assert!(AskMode::Watch.origin().contains("ブロックリストに載っていたから挙がったものではありません"));
        assert!(AskMode::Watch.origin().contains("Pi-holeが止めている"));
