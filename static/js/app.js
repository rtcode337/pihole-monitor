...
// アクセス元と期間は貯めたクエリが残っている範囲しか見られないので、そこも必ず断る ——
// 件数（Pi-hole の集計）はもっと前まで入っているため、断らないと
// 「1,400件なのにアクセス元が空」が記録漏れに見える（実際は、残っている範囲で
// 1度も止められていないだけ）
function blockedContextHtml(m) {
  ...
  return base + `<br><strong>アクセス元と期間</strong>は、手元に貯まっているクエリのうち`
    + ` Pi-hole が止めたものを<strong>全部</strong>数えたものです`
    + `（${escapeHtml(shortTimeAt(m.data_since))} 以降）。`
    + `それより前は保持期間の掃除で消えているので、件数に入っていても出ません。`
    + `<br>いま鳴っているものだけを見たいときは「1件ずつ流す」を使います。`;
}
...
function piholeQueryUrl(filter) {
  ...
  // 時間の範囲はどのモードでも付く。画面に出している期間とリンクの先を揃えるため ——
  // 食い違うと、確かめに行った意味が無い。集計の一覧は貯まっている全部を見ているので
  // `since` は「一番古い記録の時刻」、監視は窓（基準日時があればそこ）の始まり
...
// 範囲はモードで違い、前置きが断っている（集計は貯まっている全部、監視は直近24時間か
// 基準日時から）
