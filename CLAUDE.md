...
| POST | `/api/ask` | **1〜`MAX_DOMAINS_PER_ASK`件を1回の問い合わせで**聞き、結果をメモとして保存。**選んだ相手全員**に聞き、書き手（`authors`）・答えられなかった相手（`failures`）・答えの返らなかったドメイン（`missing`）を返す |
...
いまの手は3つ。**どれも「いつもと違う」の言い換え**なので、`ingest.rs`が貯めた過去が無いと成立しない。
...
- `setMode(mode)` / `renderWatchContext()` / `reasonsHtml()` - 上段のタブ（ブロック済み / 未ブロックの監視）。
