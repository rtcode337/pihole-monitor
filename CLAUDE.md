...
- `setVerdict(domain, verdict)` / `verdictBadge()` - 判定。
  **測っているのは「そのドメイン自身が問題のある通信か」**であって、
  「人が対処すべきか」ではない:

  | 値 | 意味 |
  |---|---|
  | `issue` 問題あり | 広告・トラッカー・不要なテレメトリ等。**ブロックされて当然** |
  | `ok` 問題なし | **怪しい候補として挙がったが、調べたら無害だった**（＝誤検知） |

  **「確認済み」を1つの状態にしない**のは、一覧に並ぶものが「ブロックが妥当だったもの」と
  「怪しく見えただけのもの」の混ざりもので、一語に畳むと**何が誤検知だったのかが
  分からなくなる**ため。絞り込みも `未確認 / 問題あり / 問題なし / すべて` の4つ。
  統計の2つ目は**仕分けが済んだ件数**（問題あり + 問題なし）。内訳を数字にしていた時期が
  あったが、**同じ数字なのにタブで読み方が逆になる** —— 未ブロックの監視では
  「見つけた問題の数」（多いほど困る）、ブロック済みでは「ブロックが妥当だと確かめた数」
  （多いほど順調）。ここで見たいのは「あとどれだけ仕分けが残っているか」で、
  内訳は絞り込みで見れば足りる。
  判定を送るときは**メモを送らない**（サーバ側が「渡されなければ触らない」ので、
  AIに聞いた結果や自分で書いたメモが消えない）。
  **未確認に戻すと判定も落とす**（「確認していないのに判定がある」を残さない）。
  既存DBの `reviewed=1` は移行で `ok` に倒してあるが、**これは意味の取り違えを含む**
  —— 上の定義なら広告・トラッカーの類は `issue` にあたる。移行済みのデータは
  手元の記録なので画面から付け直せばよく、**移行の規則は変えていない**
  （いま変えると、正しく付け直した `ok` まで巻き戻る）
